...
import { fireConfetti, stopConfetti } from "@/frontend/lib/confetti";
...
export function UIProvider({
  ...
  const bump = useCallback(
    async (n: number) => {
      ...
      try {
        // Through the central client like every other call — a failure still
        // just leaves the streak untouched, exactly as the bare fetch did.
        const data = await sendJson<{ streak?: number; streakExtended?: boolean }>("/api/xp", {
          xp: newXp,
          level: after,
        });
