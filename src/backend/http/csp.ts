...
/**
 * Extra origin the browser is allowed to call, when the API is NOT served from
 * this application's own origin.
 *
 * Empty today and expected to stay empty — the API is same-origin, so
 * `connect-src 'self'` covers it. It is read here so that pointing
 * `NEXT_PUBLIC_API_ORIGIN` at a company-server API host cannot silently produce
 * a policy that blocks every request the app makes; the two settings move
...
export function buildCsp(nonce: string, isDev = process.env.NODE_ENV !== "production"): string {
  const directives: string[] = [
    ...
    // The API origin is appended only when one is configured, so the default
    // policy is byte-for-byte what it has always been.
